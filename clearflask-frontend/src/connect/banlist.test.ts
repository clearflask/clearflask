...
run('/24 grouping: adjacent subnet is not banned', () => {
  for (let i = 0; i < 20; i++) recordStrike('10.10.4.1', `t${i}/`);
...
run('IPv4-mapped IPv6 (::ffff:) is grouped by /24 once normalized', () => {
  for (let i = 1; i <= 20; i++) recordStrike(normalizeIp(`::ffff:10.10.6.${i}`)!, `t${i}/`);
  assert.strictEqual(isBanned('10.10.6.99'), true);
});

run('IPv6 /64 grouping: strikes spread across /64 still ban', () => {
  for (let i = 1; i <= 20; i++) recordStrike(`2001:db8:0:1::${i.toString(16)}`, `t${i}/`);
...
run('IPv6 /64 grouping: adjacent /64 is not banned', () => {
  for (let i = 0; i < 20; i++) recordStrike('2001:db8:0:2::1', `t${i}/`);
...
run('IPv6 /64 normalization: equivalent forms hit the same bucket', () => {
  ...
  for (let i = 0; i < 19; i++) recordStrike('2001:0db8:0000:0004::1', `t${i}/`);
  recordStrike('2001:db8:0:4::abcd', 't19/');
  assert.strictEqual(isBanned('2001:db8:0:4::1'), true);
});

run('IPv6 /64: full 8-hextet form (no ::) is bucketed', () => {
  for (let i = 0; i < 20; i++) {
    recordStrike(`2001:0db8:0000:0005:0000:0000:0000:000${i.toString(16)}`, `t${i}/`);
  }
  assert.strictEqual(isBanned('2001:db8:0:5::1'), true);
});

run('IPv6 /64: zone id is stripped before bucketing', () => {
  for (let i = 0; i < 19; i++) recordStrike('fe80::1%eth0', `t${i}/`);
  recordStrike('fe80::abcd%eth1', 't19/');
...
run('IPv6 /64: malformed input falls back to per-key (no ban leakage)', () => {
  ...
  for (let i = 0; i < 19; i++) recordStrike(':::garbage', `t${i}/`);
...
run('IPv6 /64: too few segments without :: returns to per-key fallback', () => {
  for (let i = 0; i < 20; i++) recordStrike('2001:db8:bad', `t${i}/`); // only 3 segments, no ::
...
run('unique targets: refreshing the same dead link 100 times is one strike, no ban', () => {
  for (let i = 0; i < 100; i++) recordStrike('10.10.7.1', 'dead.example.com/');
  assert.strictEqual(isBanned('10.10.7.1'), false);
});

run('unique targets: repeats do not count but new targets still accumulate to a ban', () => {
  for (let i = 0; i < 50; i++) recordStrike('10.10.8.1', 'same.example.com/');
  assert.strictEqual(isBanned('10.10.8.1'), false);
  for (let i = 0; i < 19; i++) recordStrike('10.10.8.1', `scan${i}.example.com/`);
  assert.strictEqual(isBanned('10.10.8.1'), true);
});
