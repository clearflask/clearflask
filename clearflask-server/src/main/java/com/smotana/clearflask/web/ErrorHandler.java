...
public class ErrorHandler extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        log.trace("Handling error, status {} servletName {} requestUri {} throwable {}",
                statusCode, servletName, requestUri, throwableOpt);
        ...
        if ("api".equals(servletName)) {
            if (requestUri.matches("/api/?")) {
                ...
                if (throwableOpt.isPresent() && throwableOpt.get() instanceof ApiException) {
                    ApiException apiEx = (ApiException) throwableOpt.get();
                    apiEx.getUserFacingMessageOpt().ifPresent(userFacingMesssage -> {
                        out.print(gson.toJson(new ErrorResponse(userFacingMesssage)));
                        out.flush();
                    });
                }
