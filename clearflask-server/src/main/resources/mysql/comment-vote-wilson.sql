...
CREATE FUNCTION vote_wilson(
    ...
    BEGIN
        ...
        DECLARE lowerBound DOUBLE;
        -- See WilsonScoreInterval.java
        SET numberOfSuccesses = IFNULL(upvotes, 0);
        SET numberOfTrials = numberOfSuccesses + IFNULL(downvotes, 0);
        ...
        SET difference = z * SQRT((1 / numberOfTrials * mean * (1 - mean)) + (1 / (4 * POW(numberOfTrials, 2)) * zSquared));
        SET lowerBound = factor * (modifiedSuccessRatio - difference);
        RETURN lowerBound;
    END //
