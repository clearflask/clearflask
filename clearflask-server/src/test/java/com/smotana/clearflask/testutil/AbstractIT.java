...
public abstract class AbstractIT extends AbstractTest {
    ...
    private Module disabledKillBillClient() {
        return new AbstractModule() {
            @Override
            protected void configure() {
                bind(KillBillHttpClient.class).toInstance(Mockito.mock(KillBillHttpClient.class, new ThrowsException(
                        new RuntimeException("KillBill client is disabled, it should never be used")
                )));
            }
        };
    }
